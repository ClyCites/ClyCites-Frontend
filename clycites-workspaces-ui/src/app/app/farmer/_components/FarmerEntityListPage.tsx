...
export function FarmerEntityListPage({ entityKey }: { entityKey: FarmerEntityKey }) {
  ...
  const query = useQuery<EntityListCache>({
    queryKey: queryKeys.entity.list(FARMER_WORKSPACE_ID, entityKey, listParams),
  ...
  return (
    <motion.div variants={staggerContainer(Boolean(reducedMotion), 0.05)} initial="hidden" animate="show" className="space-y-4">
      <PageHeader
        ...
        breadcrumbs={[{ label: "App", href: "/app" }, { label: FARMER_WORKSPACE_ID, href: `/app/${FARMER_WORKSPACE_ID}` }, { label: definition.pluralLabel }]}
        actions={
          <>
