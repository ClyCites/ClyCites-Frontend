...
import {
  WEATHER_WORKSPACE_ID,
  type WeatherEntityKey,
  getWeatherEntityDefinition,
  getWeatherEntityFeatures,
} from "@/app/app/weather/_lib/entity-config";
import {
  buildDataPayload,
  createEmptyFormValues,
  type FormValues,
  toFormValues,
} from "@/app/app/weather/_lib/form-utils";
import { renderWeatherFieldControl } from "@/app/app/weather/_components/weather-form-controls";

interface WeatherEntityService {
...
function serviceFor(entityKey: WeatherEntityKey): WeatherEntityService {
  return entityServices[entityKey] as WeatherEntityService;
}

function entityPath(entityKey: WeatherEntityKey): string {
  return `/app/${WEATHER_WORKSPACE_ID}/${entityKey}`;
}
...
export function WeatherEntityFormPage({
  entityKey,
  mode,
  recordId,
}: {
  entityKey: WeatherEntityKey;
  mode: "create" | "edit";
  recordId?: string;
}) {
  ...
  const definition = getWeatherEntityDefinition(entityKey);
  const features = getWeatherEntityFeatures(entityKey);
  ...
  const recordQuery = useQuery<EntityRecord>({
    queryKey: ["entity", WEATHER_WORKSPACE_ID, entityKey, "record", recordId],
...
function WeatherEntityFormContent({
  entityKey,
  mode,
  recordId,
  definition,
  ...
  mode: "create" | "edit";
  recordId?: string;
  definition: ReturnType<typeof getWeatherEntityDefinition>;
  service: WeatherEntityService;
  ...
  const submitMutation = useMutation({
    ...
    onSuccess: (record) => {
      void invalidateEntityMutation(queryClient, { workspaceId: WEATHER_WORKSPACE_ID, entityKey });
  ...
  return (
    <div className="space-y-4">
      <PageHeader
        ...
        breadcrumbs={[
          { label: "App", href: "/app" },
          { label: WEATHER_WORKSPACE_ID, href: `/app/${WEATHER_WORKSPACE_ID}` },
            ...
            {definition.fields.map((field) => {
              ...
              return (
                <div key={field.key} className={field.type === "textarea" ? "space-y-1.5 md:col-span-2" : "space-y-1.5"}>
                  ...
                  {renderWeatherFieldControl(field, value, (next) => setFormValues((current) => ({ ...current, [field.key]: next })))}
                </div>
              );
            })}
          </div>
