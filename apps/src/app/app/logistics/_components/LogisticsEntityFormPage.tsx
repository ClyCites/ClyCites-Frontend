...
import {
  ...
  type FormValues,
  toFormValues,
} from "@/app/app/logistics/_lib/form-utils";
import { renderLogisticsFieldControl } from "@/app/app/logistics/_components/logistics-form-controls";
...
export function LogisticsEntityFormPage({
  entityKey,
  mode,
  recordId,
}: {
  entityKey: LogisticsEntityKey;
  mode: "create" | "edit";
  recordId?: string;
}) {
  ...
  const recordQuery = useQuery<EntityRecord>({
    queryKey: ["entity", LOGISTICS_WORKSPACE_ID, entityKey, "record", recordId],
...
function LogisticsEntityFormContent({
  entityKey,
  mode,
  recordId,
  definition,
  ...
  mode: "create" | "edit";
  recordId?: string;
  definition: ReturnType<typeof getLogisticsEntityDefinition>;
  service: LogisticsEntityService;
  ...
  return (
    <div className="space-y-4">
      <PageHeader
        ...
        breadcrumbs={[
          { label: "App", href: "/app" },
          { label: LOGISTICS_WORKSPACE_ID, href: `/app/${LOGISTICS_WORKSPACE_ID}` },
            ...
            {definition.fields.map((field) => {
              ...
              return (
                <div key={field.key} className={field.type === "textarea" ? "space-y-1.5 md:col-span-2" : "space-y-1.5"}>
                  ...
                  {renderLogisticsFieldControl(field, value, (next) => setFormValues((current) => ({ ...current, [field.key]: next })))}
                </div>
              );
            })}
          </div>
