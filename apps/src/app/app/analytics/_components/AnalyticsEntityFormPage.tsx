...
import {
  ...
  type AnalyticsEntityKey,
  getAnalyticsEntityDefinition,
  getAnalyticsEntityFeatures,
} from "@/app/app/analytics/_lib/entity-config";
import {
  buildDataPayload,
  createEmptyFormValues,
  type FormValues,
  toFormValues,
} from "@/app/app/analytics/_lib/form-utils";
import { renderAnalyticsFieldControl } from "@/app/app/analytics/_components/analytics-form-controls";

interface AnalyticsEntityService {
...
function serviceFor(entityKey: AnalyticsEntityKey): AnalyticsEntityService {
  return entityServices[entityKey] as AnalyticsEntityService;
}

function entityPath(entityKey: AnalyticsEntityKey): string {
  return `/app/${ANALYTICS_WORKSPACE_ID}/${entityKey}`;
}
...
export function AnalyticsEntityFormPage({
  entityKey,
  mode,
  recordId,
}: {
  ...
  const definition = getAnalyticsEntityDefinition(entityKey);
  const features = getAnalyticsEntityFeatures(entityKey);
  ...
  const recordQuery = useQuery<EntityRecord>({
    queryKey: ["entity", ANALYTICS_WORKSPACE_ID, entityKey, "record", recordId],
...
function AnalyticsEntityFormContent({
  entityKey,
  mode,
  recordId,
  definition,
  ...
  mode: "create" | "edit";
  recordId?: string;
  definition: ReturnType<typeof getAnalyticsEntityDefinition>;
  service: AnalyticsEntityService;
  ...
  const submitMutation = useMutation({
    ...
    onSuccess: (record) => {
      void invalidateEntityMutation(queryClient, { workspaceId: ANALYTICS_WORKSPACE_ID, entityKey });
  ...
  return (
    <div className="space-y-4">
      <PageHeader
        ...
        breadcrumbs={[
          { label: "App", href: "/app" },
          { label: ANALYTICS_WORKSPACE_ID, href: `/app/${ANALYTICS_WORKSPACE_ID}` },
            ...
            {definition.fields.map((field) => {
              ...
              return (
                <div key={field.key} className={field.type === "textarea" ? "space-y-1.5 md:col-span-2" : "space-y-1.5"}>
                  ...
                  {renderAnalyticsFieldControl(field, value, (next) => setFormValues((current) => ({ ...current, [field.key]: next })))}
                </div>
              );
            })}
          </div>
