...
export function AnalyticsEntityListPage({ entityKey }: { entityKey: AnalyticsEntityKey }) {
  ...
  const query = useQuery<EntityListCache>({
    queryKey: queryKeys.entity.list(ANALYTICS_WORKSPACE_ID, entityKey, listParams),
  ...
  return (
    <motion.div variants={staggerContainer(Boolean(reducedMotion), 0.05)} initial="hidden" animate="show" className="space-y-4">
      <PageHeader
        ...
        breadcrumbs={[{ label: "App", href: "/app" }, { label: ANALYTICS_WORKSPACE_ID, href: `/app/${ANALYTICS_WORKSPACE_ID}` }, { label: definition.pluralLabel }]}
        actions={
          <>
