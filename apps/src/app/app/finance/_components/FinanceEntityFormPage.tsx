...
import {
  ...
  type FinanceEntityKey,
  getFinanceEntityDefinition,
  getFinanceEntityFeatures,
} from "@/app/app/finance/_lib/entity-config";
import {
  buildDataPayload,
  createEmptyFormValues,
  type FormValues,
  toFormValues,
} from "@/app/app/finance/_lib/form-utils";
import { renderFinanceFieldControl } from "@/app/app/finance/_components/finance-form-controls";
...
function serviceFor(entityKey: FinanceEntityKey): FinanceEntityService {
  return entityServices[entityKey] as FinanceEntityService;
}

function entityPath(entityKey: FinanceEntityKey): string {
  return `/app/${FINANCE_WORKSPACE_ID}/${entityKey}`;
}
...
export function FinanceEntityFormPage({
  entityKey,
  mode,
  recordId,
}: {
  entityKey: FinanceEntityKey;
  mode: "create" | "edit";
  recordId?: string;
}) {
  ...
  const definition = getFinanceEntityDefinition(entityKey);
  const features = getFinanceEntityFeatures(entityKey);
  ...
  const recordQuery = useQuery<EntityRecord>({
    queryKey: ["entity", FINANCE_WORKSPACE_ID, entityKey, "record", recordId],
...
function FinanceEntityFormContent({
  entityKey,
  mode,
  recordId,
  definition,
  ...
  mode: "create" | "edit";
  recordId?: string;
  definition: ReturnType<typeof getFinanceEntityDefinition>;
  service: FinanceEntityService;
  ...
  const submitMutation = useMutation({
    ...
    onSuccess: (record) => {
      void invalidateEntityMutation(queryClient, { workspaceId: FINANCE_WORKSPACE_ID, entityKey });
  ...
  return (
    <div className="space-y-4">
      <PageHeader
        ...
        breadcrumbs={[
          { label: "App", href: "/app" },
          { label: FINANCE_WORKSPACE_ID, href: `/app/${FINANCE_WORKSPACE_ID}` },
            ...
            {definition.fields.map((field) => {
              ...
              return (
                <div key={field.key} className={field.type === "textarea" ? "space-y-1.5 md:col-span-2" : "space-y-1.5"}>
                  ...
                  {renderFinanceFieldControl(field, value, (next) => setFormValues((current) => ({ ...current, [field.key]: next })))}
                </div>
              );
            })}
          </div>
