...
export function AdminEntityListPage({ entityKey }: { entityKey: AdminEntityKey }) {
  ...
  const query = useQuery<EntityListCache>({
    queryKey: queryKeys.entity.list(ADMIN_WORKSPACE_ID, entityKey, listParams),
  ...
  return (
    <motion.div variants={staggerContainer(Boolean(reducedMotion), 0.05)} initial="hidden" animate="show" className="space-y-4">
      <PageHeader
        ...
        breadcrumbs={[{ label: "App", href: "/app" }, { label: ADMIN_WORKSPACE_ID, href: `/app/${ADMIN_WORKSPACE_ID}` }, { label: definition.pluralLabel }]}
        actions={
          <>
