...
import {
  ...
  getAdminEntityFeatures,
} from "@/app/app/admin/_lib/entity-config";
import {
  buildDataPayload,
  createEmptyFormValues,
  type FormValues,
  toFormValues,
} from "@/app/app/admin/_lib/form-utils";
import { renderAdminFieldControl } from "@/app/app/admin/_components/admin-form-controls";
...
function serviceFor(entityKey: AdminEntityKey): AdminEntityService {
  return entityServices[entityKey] as AdminEntityService;
}

function entityPath(entityKey: AdminEntityKey): string {
  return `/app/${ADMIN_WORKSPACE_ID}/${entityKey}`;
}
...
export function AdminEntityFormPage({
  ...
  const features = getAdminEntityFeatures(entityKey);
  ...
  const recordQuery = useQuery<EntityRecord>({
    queryKey: ["entity", ADMIN_WORKSPACE_ID, entityKey, "record", recordId],
...
function AdminEntityFormContent({
  entityKey,
  mode,
  recordId,
  definition,
  ...
  mode: "create" | "edit";
  recordId?: string;
  definition: ReturnType<typeof getAdminEntityDefinition>;
  service: AdminEntityService;
  ...
  const submitMutation = useMutation({
    ...
    onSuccess: (record) => {
      void invalidateEntityMutation(queryClient, { workspaceId: ADMIN_WORKSPACE_ID, entityKey });
  ...
  return (
    <div className="space-y-4">
      <PageHeader
        ...
        breadcrumbs={[
          { label: "App", href: "/app" },
          { label: ADMIN_WORKSPACE_ID, href: `/app/${ADMIN_WORKSPACE_ID}` },
            ...
            {definition.fields.map((field) => {
              ...
              return (
                <div key={field.key} className={field.type === "textarea" ? "space-y-1.5 md:col-span-2" : "space-y-1.5"}>
                  ...
                  {renderAdminFieldControl(field, value, (next) => setFormValues((current) => ({ ...current, [field.key]: next })))}
                </div>
              );
            })}
          </div>
