...
import {
  EXPERT_WORKSPACE_ID,
  type ExpertEntityKey,
  getExpertEntityDefinition,
  getExpertEntityFeatures,
} from "@/app/app/expert/_lib/entity-config";
import {
  buildDataPayload,
  createEmptyFormValues,
  type FormValues,
  toFormValues,
} from "@/app/app/expert/_lib/form-utils";
import { renderExpertFieldControl } from "@/app/app/expert/_components/expert-form-controls";

interface ExpertEntityService {
...
function serviceFor(entityKey: ExpertEntityKey): ExpertEntityService {
  return entityServices[entityKey] as ExpertEntityService;
}

function entityPath(entityKey: ExpertEntityKey): string {
  return `/app/${EXPERT_WORKSPACE_ID}/${entityKey}`;
}
...
export function ExpertEntityFormPage({
  entityKey,
  mode,
  recordId,
}: {
  entityKey: ExpertEntityKey;
  mode: "create" | "edit";
  recordId?: string;
}) {
  ...
  const definition = getExpertEntityDefinition(entityKey);
  const features = getExpertEntityFeatures(entityKey);
  ...
  const recordQuery = useQuery<EntityRecord>({
    queryKey: ["entity", EXPERT_WORKSPACE_ID, entityKey, "record", recordId],
...
function ExpertEntityFormContent({
  entityKey,
  mode,
  recordId,
  definition,
  ...
  mode: "create" | "edit";
  recordId?: string;
  definition: ReturnType<typeof getExpertEntityDefinition>;
  service: ExpertEntityService;
  ...
  const submitMutation = useMutation({
    ...
    onSuccess: (record) => {
      void invalidateEntityMutation(queryClient, { workspaceId: EXPERT_WORKSPACE_ID, entityKey });
  ...
  return (
    <div className="space-y-4">
      <PageHeader
        ...
        breadcrumbs={[
          { label: "App", href: "/app" },
          { label: EXPERT_WORKSPACE_ID, href: `/app/${EXPERT_WORKSPACE_ID}` },
            ...
            {definition.fields.map((field) => {
              ...
              return (
                <div key={field.key} className={field.type === "textarea" ? "space-y-1.5 md:col-span-2" : "space-y-1.5"}>
                  ...
                  {renderExpertFieldControl(field, value, (next) => setFormValues((current) => ({ ...current, [field.key]: next })))}
                </div>
              );
            })}
          </div>
