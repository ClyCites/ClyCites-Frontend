...
import {
  AGGREGATION_WORKSPACE_ID,
  type AggregationEntityKey,
  getAggregationEntityDefinition,
  getAggregationEntityFeatures,
} from "@/app/app/aggregation/_lib/entity-config";
import {
  buildDataPayload,
  createEmptyFormValues,
  type FormValues,
  toFormValues,
} from "@/app/app/aggregation/_lib/form-utils";
import { renderAggregationFieldControl } from "@/app/app/aggregation/_components/aggregation-form-controls";

interface AggregationEntityService {
...
function serviceFor(entityKey: AggregationEntityKey): AggregationEntityService {
  return entityServices[entityKey] as AggregationEntityService;
}

function entityPath(entityKey: AggregationEntityKey): string {
  return `/app/${AGGREGATION_WORKSPACE_ID}/${entityKey}`;
}
...
export function AggregationEntityFormPage({
  entityKey,
  mode,
  recordId,
}: {
  entityKey: AggregationEntityKey;
  mode: "create" | "edit";
  recordId?: string;
}) {
  ...
  const definition = getAggregationEntityDefinition(entityKey);
  const features = getAggregationEntityFeatures(entityKey);
  ...
  const recordQuery = useQuery<EntityRecord>({
    queryKey: ["entity", AGGREGATION_WORKSPACE_ID, entityKey, "record", recordId],
...
function AggregationEntityFormContent({
  entityKey,
  mode,
  recordId,
  definition,
  ...
  mode: "create" | "edit";
  recordId?: string;
  definition: ReturnType<typeof getAggregationEntityDefinition>;
  service: AggregationEntityService;
  ...
  const submitMutation = useMutation({
    ...
    onSuccess: (record) => {
      void invalidateEntityMutation(queryClient, { workspaceId: AGGREGATION_WORKSPACE_ID, entityKey });
  ...
  return (
    <div className="space-y-4">
      <PageHeader
        ...
        breadcrumbs={[
          { label: "App", href: "/app" },
          { label: AGGREGATION_WORKSPACE_ID, href: `/app/${AGGREGATION_WORKSPACE_ID}` },
            ...
            {definition.fields.map((field) => {
              ...
              return (
                <div key={field.key} className={field.type === "textarea" ? "space-y-1.5 md:col-span-2" : "space-y-1.5"}>
                  ...
                  {renderAggregationFieldControl(field, value, (next) => setFormValues((current) => ({ ...current, [field.key]: next })))}
                </div>
              );
            })}
          </div>
