...
import {
  ...
  type FormValues,
  toFormValues,
} from "@/app/app/production/_lib/form-utils";
import { renderProductionFieldControl } from "@/app/app/production/_components/production-form-controls";
...
export function ProductionEntityFormPage({
  entityKey,
  mode,
  recordId,
}: {
  entityKey: ProductionEntityKey;
  mode: "create" | "edit";
  recordId?: string;
}) {
...
function ProductionEntityFormContent({
  entityKey,
  mode,
  recordId,
  definition,
  ...
  mode: "create" | "edit";
  recordId?: string;
  definition: ReturnType<typeof getProductionEntityDefinition>;
  service: ProductionEntityService;
            ...
            {definition.fields.map((field) => {
              ...
              return (
                <div key={field.key} className={field.type === "textarea" ? "space-y-1.5 md:col-span-2" : "space-y-1.5"}>
                  ...
                  {renderProductionFieldControl(field, value, (next) => setFormValues((current) => ({ ...current, [field.key]: next })))}
                </div>
              );
            })}
          </div>
