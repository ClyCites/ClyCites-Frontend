...
import {
  ...
  type MarketplaceEntityKey,
  getMarketplaceEntityDefinition,
  getMarketplaceEntityFeatures,
} from "@/app/app/marketplace/_lib/entity-config";
import {
  buildDataPayload,
  createEmptyFormValues,
  type FormValues,
  toFormValues,
} from "@/app/app/marketplace/_lib/form-utils";
import { renderMarketplaceFieldControl } from "@/app/app/marketplace/_components/marketplace-form-controls";

interface MarketplaceEntityService {
...
function serviceFor(entityKey: MarketplaceEntityKey): MarketplaceEntityService {
  return entityServices[entityKey] as MarketplaceEntityService;
}

function entityPath(entityKey: MarketplaceEntityKey): string {
  return `/app/${MARKETPLACE_WORKSPACE_ID}/${entityKey}`;
}
...
export function MarketplaceEntityFormPage({
  entityKey,
  mode,
  recordId,
}: {
  entityKey: MarketplaceEntityKey;
  mode: "create" | "edit";
  recordId?: string;
}) {
  ...
  const definition = getMarketplaceEntityDefinition(entityKey);
  const features = getMarketplaceEntityFeatures(entityKey);
  ...
  const recordQuery = useQuery<EntityRecord>({
    queryKey: ["entity", MARKETPLACE_WORKSPACE_ID, entityKey, "record", recordId],
...
function MarketplaceEntityFormContent({
  entityKey,
  mode,
  recordId,
  definition,
  ...
  mode: "create" | "edit";
  recordId?: string;
  definition: ReturnType<typeof getMarketplaceEntityDefinition>;
  service: MarketplaceEntityService;
  ...
  const submitMutation = useMutation({
    ...
    onSuccess: (record) => {
      void invalidateEntityMutation(queryClient, { workspaceId: MARKETPLACE_WORKSPACE_ID, entityKey });
  ...
  return (
    <div className="space-y-4">
      <PageHeader
        ...
        breadcrumbs={[
          { label: "App", href: "/app" },
          { label: MARKETPLACE_WORKSPACE_ID, href: `/app/${MARKETPLACE_WORKSPACE_ID}` },
            ...
            {definition.fields.map((field) => {
              ...
              return (
                <div key={field.key} className={field.type === "textarea" ? "space-y-1.5 md:col-span-2" : "space-y-1.5"}>
                  ...
                  {renderMarketplaceFieldControl(field, value, (next) => setFormValues((current) => ({ ...current, [field.key]: next })))}
                </div>
              );
            })}
          </div>
